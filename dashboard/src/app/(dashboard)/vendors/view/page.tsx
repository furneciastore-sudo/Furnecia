...
import { getSettings, getVendor } from "@/lib/localApi";
...
function VendorDetailInner() {
  ...
  const [vendor, setVendor] = useState<ReturnType<typeof getVendor> | null>(null);
  ...
  if (notFound) return <p className="text-sm text-gray-500">Vendor not found.</p>;
  if (!vendor) return <p className="text-sm text-gray-400">Loading…</p>;
        ...
        {vendor.orders.length === 0 ? (
          ...
          <table className="min-w-[900px] w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-gray-100">
              {vendor.orders.map((o) => (
                <tr key={o.id}>
                  <td className="py-1.5"><Link href={`/orders/view?id=${o.id}`} className="font-medium text-brand-700">{o.orderNo}</Link></td>
